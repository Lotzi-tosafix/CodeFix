...
תלמידים רבים מתבלבלים בתחילת הדרך מהסימנים המיוחדים ב-HTML. למה יש שווה (\`=\`)? מתי צריך גרשיים (\`""\`)? ומתי משתמשים בלוכסן (\`/\`)?
השיעור הזה הוא המילון שלכם. נפרק כל סימן ונסביר את תפקידו המדויק.

## 1. הסוגריים המשולשים: \`< >\` (הפקודה)

זהו הבסיס. כל הוראה שאנחנו נותנים לדפדפן חייבת להיות עטופה בסימנים האלו.
*   **המשמעות:** "דפדפן יקר, שים לב! מה שכתוב כאן הוא פקודת קוד, לא סתם טקסט רגיל שצריך להציג על המסך".

**דוגמה:**
\`<button>\`
הדפדפן רואה \`<\`, נכנס למצב "קריאת פקודה", קורא את המילה \`button\`, וברגע שהוא רואה \`>\` הוא מבין: "סיימתי לקרוא את שם הפקודה, עכשיו אני מבצע ויוצר כפתור".

## 2. הלוכסן: \`/\` (התמרור "עצור")

הסימן הזה אומר לדפדפן שמשהו **נגמר**. אנחנו משתמשים בו בתגית סגירה.
...
**דוגמה:**
\`</h1>\`
הדפדפן מבין שהכותרת הגדולה שהתחילה קודם, הסתיימה כאן.

---

## 3. השווה: \`=\` (הגדרות ותכונות)

כאן אנחנו נכנסים לנושא חשוב שנקרא **תכונות** (Attributes).
תחשבו על זה ככה: לפעמים התגית עצמה לא מספיקה.
למשל, אם נגיד לדפדפן "צור קישור" (\`<a>\`), הוא ישאל אותנו: "לאן הקישור מוביל?".
אם נגיד לו "שים תמונה" (\`<img>\`), הוא ישאל: "איזו תמונה?".

אנחנו צריכים דרך לתת **הגדרות נוספות** לתגית. ההגדרות האלו נקראות **Attributes**, והסימן שווה (\`=\`) הוא הדרך שלנו להצמיד ערך להגדרה.
...
\`\`\`
כאן אנחנו אומרים לדפדפן: צור שדה קלט (\`input\`), וההגדרה שלו היא שהערך ההתחלתי (\`value\`) יהיה "ישראל ישראלי".

## 4. הגרשיים: \`"..."\` (הדבק / האריזה)

למה אנחנו צריכים לעטוף את הערך בגרשיים? למה אי אפשר פשוט לכתוב \`value=ישראל ישראלי\`?

הסיבה היא שהמחשב לא יודע לקרוא רווחים כמו בני אדם.
בלי גרשיים, המחשב יחשוב שהערך הוא רק המילה הראשונה ("ישראל"), ולא יבין מה לעשות עם המילה השנייה ("ישראלי"). הוא יחשוב שזו פקודה חדשה וזה יגרום לשגיאה.

הגרשיים משמשים כמו **דבק** או **שקית אריזה**. הם אומרים לדפדפן: "כל מה שנמצא בתוכנו – גם אם יש בו רווחים – הוא דבר אחד שלם".

### רגע, אז למה לא שמים גרשיים בתוכן?
שאלה מצוינת! שימו לב להבדל:

**בתוך כפתור (התוכן) - אין גרשיים:**
\`\`\`html
<button>לחץ עליי עכשיו</button>
\`\`\`
כאן **אין צורך בגרשיים** סביב הטקסט "לחץ עליי עכשיו", כי יש לנו את תגיות הפתיחה והסגירה שמשמשות כ"קירות" או גבולות גזרה. הדפדפן יודע שכל מה שבין ה-\`<button>\` לבין ה-\`</button>\` הוא התוכן.

**בתוך הגדרה (Attribute) - יש גרשיים:**
\`\`\`html
<input placeholder="הקלד שם כאן">
\`\`\`
כאן ההגדרה נמצאת **בתוך** תגית הפתיחה עצמה. אין שם תגית סגירה שתשמש כגבול, ולכן אנחנו חייבים את הגרשיים כדי לסמן איפה הטקסט של ההגדרה מתחיל ואיפה הוא נגמר.
...
\`\`\`html
  <input   placeholder  =  "הקלד כאן"  >
    ^           ^       ^      ^       ^
  פקודה       הגדרה   משווה   ערך    סוף פקודה
\`\`\`

1.  **הפקודה:** צור שדה קלט.
2.  **ההגדרה:** אני רוצה להגדיר את הטקסט המקדים (placeholder).
3.  **השווה:** הערך של ההגדרה הזו הוא...
4.  **הערך:** "הקלד כאן" (עטוף בגרשיים כדי שיהיה יחידה אחת).
    `,
    practice: [
      {
        type: 'quiz',
        id: 'q_syntax_1',
        question: 'מדוע אנחנו עוטפים את ערך התכונה (Attribute) בגרשיים? (למשל `placeholder="שלום עולם"`)',
        options: ['כדי להדגיש את הטקסט', 'כדי שהדפדפן ידע שהערך הוא יחידה אחת שלמה, גם אם יש בו רווחים', 'זה רק בשביל היופי, זה לא חובה'],
        correctAnswer: 1,
        explanation: 'הגרשיים "אורזים" את המידע כדי שהדפדפן ידע שכל מה שבפנים שייך לאותה הגדרה, ומסמנים איפה היא מתחילה ונגמרת.'
      },
      {
        type: 'quiz',
        id: 'q_syntax_2',
        question: 'האם צריך לשים גרשיים סביב טקסט שנמצא **בין** תגית פתיחה לתגית סגירה? (למשל בתוך כפתור)',
        options: ['לא, כי התגיות עצמן (< >) משמשות כגבולות שמגדירים איפה התוכן מתחיל ונגמר', 'כן, תמיד צריך גרשיים בכל מקום', 'רק אם הטקסט בעברית'],
        correctAnswer: 0,
        explanation: 'בתוכן האלמנט (בין התגיות), התגיות עצמן הן הגבול. גרשיים נדרשים רק כאשר נותנים ערך להגדרה (Attribute) בתוך תגית הפתיחה.'
      }
    ]
  },
  'html-structure': {
