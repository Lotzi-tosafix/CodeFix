...
const ProfileModal: React.FC<ProfileModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div className="relative bg-slate-900 border border-slate-700 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-float">
        
        {/* Header */}
        <div className="bg-gradient-to-r from-brand-900/50 to-slate-900 p-6 border-b border-slate-700 flex justify-between items-center">
        ...
        <div className="overflow-y-auto p-6 custom-scrollbar">
            ...
            {/* User Info */}
            <div className="flex flex-col md:flex-row items-center gap-6 mb-10 pb-8 border-b border-slate-800">
                ...
                <div className="text-center md:text-start">
                    <h3 className="text-xl font-bold text-white">{t.profile.hello}, {user.name}!</h3>
                    <p className="text-slate-400 mb-4">{user.email}</p>
                    <button 
                        onClick={onLogout}
                        className="inline-flex items-center px-4 py-2 bg-slate-800 hover:bg-red-900/30 text-slate-300 hover:text-red-400 rounded-full text-sm font-medium transition-colors border border-slate-700 hover:border-red-500/30"
                    >
            ...
            <div>
                ...
                {completedLessons.length === 0 ? (
                    <div className="text-center py-10 bg-slate-800/30 rounded-xl border border-dashed border-slate-700">
                        <p className="text-slate-500">{t.profile.noProgress}</p>
                    </div>
                ) : (
                    <div className="space-y-3 mb-8">
                        {completedLessons.map(lessonId => {
                            ...
                            return (
                                <div key={lessonId} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg border border-slate-700 hover:border-slate-600 transition-colors group">
                    ...
                    <div className="mt-8 pt-8 border-t border-slate-800">
                        {!showResetConfirm ? (
                            <button 
                                ...
                                className="w-full flex items-center justify-center p-3 text-red-400 border border-red-900/50 rounded-xl hover:bg-red-900/10 transition-colors"
                            >
                                <AlertTriangle size={18} className="mr-2 rtl:mr-0 rtl:ml-2" />
                                {t.profile.resetAll}
                            </button>
                        ) : (
                            <div className="bg-red-900/20 border border-red-500/30 p-4 rounded-xl text-center animate-fade-in-up">
                                <ShieldAlert size={32} className="text-red-500 mx-auto mb-2" />
                                <p className="text-red-200 mb-4 font-bold">{t.profile.resetConfirm}</p>
