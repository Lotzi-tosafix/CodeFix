...
import { Globe, Menu, X, Code2, LogIn, User as UserIcon, Sun, Moon, ShieldCheck } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center">
            <div className="flex items-center space-x-2 rtl:space-x-reverse">
              ...
              {user?.isAdmin && (
                <Link to="/admin" className="flex items-center cursor-pointer px-3 py-2 rounded-md text-sm font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/10 hover:bg-red-100 transition-colors">
                    <ShieldCheck size={16} className="mr-1 rtl:ml-1 rtl:mr-0" />
                    {t.nav.admin}
                </Link>
              )}
      ...
      {isOpen && (
        <div className="md:hidden bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <button onClick={() => {navigate('/contact'); setIsOpen(false)}} className="block w-full text-start text-slate-600 dark:text-slate-300 hover:text-brand-600 dark:hover:text-white px-3 py-2 rounded-md text-base font-medium">
              {t.nav.contact}
            </button>

             {user?.isAdmin && (
                <button onClick={() => {navigate('/admin'); setIsOpen(false)}} className="block w-full text-start text-red-600 dark:text-red-400 hover:text-red-500 font-bold px-3 py-2 rounded-md text-base">
                  {t.nav.admin}
                </button>
             )}
