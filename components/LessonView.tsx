...
const LessonView: React.FC<LessonViewProps> = ({ 
  ...
  // 1. Text Splitter - הגרסה המלוטשת ביותר
  const splitTextToSentences = (text: string): string[] => {
    if (!text) return [];
    
    let clean = text
      // 1. הסרת בלוקים של קוד לחלוטין
      .replace(/```[\s\S]*?```/g, ' קוד דוגמה. ')
      
      // 2. טיפול בלוכסנים (דפדפן / Browser) -> (דפדפן או Browser)
      // מחליפים רק לוכסן שיש סביבו רווחים, כדי לא להרוס תאריכים או כתובות אם נשארו
      .replace(/\s\/\s/g, ' או ') 

      // 3. הסרת אימוג'ים (טווח יוניקוד רחב)
      .replace(/[\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu, '')

      // 4. הסרת כותרות (#) ומקפים בתחילת שורה
      .replace(/#+/g, '') 
      .replace(/^[\s-]*[-*]\s+/gm, '') 
      
      // 5. ניקוי סימני Markdown + סוגריים!
      // הסרנו את () כדי שלא יגיד "פתח סוגריים"
      .replace(/[*_`>\[\]()]/g, '') 
      
      // 6. הסרת תמונות וקישורים (משאיר רק את הטקסט של הקישור)
      .replace(/!\[.*?\]\(.*?\)/g, '') 
      .replace(/\[(.*?)\]\(.*?\)/g, '$1') 
      
      // 7. טיפול בנקודותיים - הופך לנקודה (הפסקה בדיבור)
      .replace(/:/g, '. ')

      // 8. שורות חדשות הופכות לנקודות
      .replace(/\n+/g, '. '); 

    // פיצול למשפטים וניקוי רווחים כפולים שנוצרו
    return clean
      .match(/[^.?!]+[.?!]+["']?|[^.?!]+$/g) || []
      .map(s => s.trim())
      .filter(s => s.length > 2); // סינון משפטים קצרים מדי
  };
