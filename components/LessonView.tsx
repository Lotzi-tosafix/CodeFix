...
const LessonView: React.FC<LessonViewProps> = ({ 
  ...
  // 1. Text Splitter - גרסה מתוקנת עם הפסקות בסוגריים
  const splitTextToSentences = (text: string): string[] => {
    if (!text) return [];
    
    let clean = text
      // 1. הסרת בלוקים של קוד
      ...
      // 5. טיפול בסוגריים: הופכים לפסיקים כדי ליצור הפסקה
      .replace(/[()]/g, ', ') 
      
      // 6. ניקוי שאר סימני Markdown (ללא סוגריים כי טיפלנו בהם)
      .replace(/[*_`>\[\]]/g, '') 
      
      // 7. הסרת תמונות וקישורים (משאיר רק טקסט)
      .replace(/!\[.*?\]\(.*?\)/g, '') 
      .replace(/\[(.*?)\]\(.*?\)/g, '$1') 
      
      // 8. נקודותיים -> נקודה (הפסקה ארוכה יותר)
      .replace(/:/g, '. ')

      // 9. שורות חדשות -> נקודה
      .replace(/\n+/g, '. ')

      // 10. ניקוי רעשים שנוצרו מההחלפות (כמו פסיק ומיד אחריו נקודה)
      .replace(/,\s*\./g, '.')  // פסיק ואז נקודה -> רק נקודה
      .replace(/\.\s*,/g, '.')  // נקודה ואז פסיק -> רק נקודה
      .replace(/,\s*,/g, ', '); // פסיק כפול -> פסיק אחד
