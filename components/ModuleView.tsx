                ...
                <div>
                    <div className="flex items-center gap-3 mb-1">
                         <span className={`
                            ...
                            ${module.levelKey === 'beginner' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 border border-green-500/20' : ''}
                            ${module.levelKey === 'intermediate' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 border border-yellow-500/20' : ''}
                            ${module.levelKey === 'advanced' ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-500/20' : ''}
                        `}>
                    ...
                    <h1 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white mb-2">{t.curriculum.modules[module.titleKey]}</h1>
                    <p className="text-slate-600 dark:text-slate-400 max-w-xl">{module.description}</p>
                </div>
            </div>

            {/* Progress Card */}
            <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-5 rounded-2xl min-w-[280px] shadow-sm">
                <div className="flex justify-between items-end mb-2">
                    <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">{t.module.progress}</span>
                ...
                <div className="mt-2 text-xs text-slate-500 dark:text-slate-500 text-end">
            ...
            return (
                <div 
                    ...
                    className={`
                        ...
                        ${isClickable 
                            ? 'bg-white dark:bg-dark-card border-slate-200 dark:border-slate-700 hover:border-brand-500 dark:hover:border-brand-500 hover:shadow-xl hover:-translate-y-1 cursor-pointer' 
                ...
                >
                    <div className="flex justify-between items-start mb-4">
                        <div className={`
                            ...
                            ${isCompleted 
                                ...
                                : isClickable 
                                    ? 'bg-slate-100 dark:bg-slate-700 text-brand-600 dark:text-brand-200 group-hover:bg-brand-600 group-hover:text-white transition-colors' 
                                    : 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-600'}
                        `}>
                            {isCompleted ? <CheckCircle size={20} /> : index + 1}
                        </div>
                        {isCompleted && (
                             <span className="text-xs font-bold text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/20 px-2 py-1 rounded-full border border-green-500/20">
                                {t.module.completed}
                             </span>
                        )}
                        {!isClickable && <Lock size={18} className="text-slate-400 dark:text-slate-600" />}
                    </div>

                    <h3 className={`text-xl font-bold mb-3 ${isClickable ? 'text-slate-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-300 transition-colors' : 'text-slate-400 dark:text-slate-500'}`}>
                        {lesson.title}
                    </h3>
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-700/50 mt-auto flex items-center justify-between">
                         ...
                         {isClickable && (
                            <div className="bg-slate-100 dark:bg-slate-800 p-2 rounded-full text-slate-400 group-hover:bg-brand-500 group-hover:text-white transition-all">
